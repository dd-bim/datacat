...
import de.bentrm.datacat.catalog.service.value.verification.findMissingDescriptionValue;
...
import de.bentrm.datacat.graphql.input.verification.findMissingDescriptionFilterInput;
import de.bentrm.datacat.graphql.input.verification.findMissingDescriptionNodeTypeFilterInput;
...
@Slf4j
@Component
public class findMissingDescriptionFetchers implements QueryFetchers {
    ...
    public Map<String, DataFetcher> getQueryFetchers() {
        return Map.ofEntries(
                ...
                Map.entry("findMissingDescription", findMissingDescription())
        );
    }
    ...
    public DataFetcher<findMissingDescriptionValue> findMissingDescription() {
        return environment -> {
            Map<String, Object> argument = environment.getArgument("input");
            final findMissingDescriptionFilterInput input = inputMapper.tofindMissingDescriptionFilterInput(argument);
            final findMissingDescriptionNodeTypeFilterInput nodeTypeFilter = input.getNodeTypeFilter();
            final CatalogRecordSpecification catalogEntryType = specificationMapper.toCatalogRecordSpecification(nodeTypeFilter);
            return catalogVerificationService.getfindMissingDescription(catalogEntryType);
        };
    }
