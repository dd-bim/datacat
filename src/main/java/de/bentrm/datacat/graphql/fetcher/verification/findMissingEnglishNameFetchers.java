...
import de.bentrm.datacat.catalog.service.value.verification.findMissingEnglishNameValue;
...
import de.bentrm.datacat.graphql.input.verification.findMissingEnglishNameFilterInput;
import de.bentrm.datacat.graphql.input.verification.findMissingEnglishNameNodeTypeFilterInput;
...
@Slf4j
@Component
public class findMissingEnglishNameFetchers implements QueryFetchers {
    ...
    public Map<String, DataFetcher> getQueryFetchers() {
        return Map.ofEntries(
                ...
                Map.entry("findMissingEnglishName", findMissingEnglishName())
        );
    }
    ...
    public DataFetcher<findMissingEnglishNameValue> findMissingEnglishName() {
        return environment -> {
            Map<String, Object> argument = environment.getArgument("input");
            final findMissingEnglishNameFilterInput input = inputMapper.tofindMissingEnglishNameFilterInput(argument);
            final findMissingEnglishNameNodeTypeFilterInput nodeTypeFilter = input.getNodeTypeFilter();
            final CatalogRecordSpecification catalogEntryType = specificationMapper.toCatalogRecordSpecification(nodeTypeFilter);
            return catalogVerificationService.getfindMissingEnglishName(catalogEntryType);
        };
    }
