...
import de.bentrm.datacat.catalog.service.value.verification.findSubjectWithoutPropValue;
...
@Slf4j
@Component
public class findSubjectWithoutPropFetchers implements QueryFetchers {
    ...
    public Map<String, DataFetcher> getQueryFetchers() {
        return Map.ofEntries(
                ...
                Map.entry("findSubjectWithoutProp", findSubjectWithoutProp())
        );
    }
    ...
    public DataFetcher<findSubjectWithoutPropValue> findSubjectWithoutProp() {
        return environment -> {
            return catalogVerificationService.getfindSubjectWithoutProp();
        };
    };
}
