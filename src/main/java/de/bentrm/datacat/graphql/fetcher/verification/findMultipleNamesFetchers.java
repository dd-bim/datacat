...
import de.bentrm.datacat.graphql.input.verification.findMultipleNamesFilterInput;
import de.bentrm.datacat.graphql.input.verification.findMultipleNamesNodeTypeFilterInput;
...
@Slf4j
@Component
public class findMultipleNamesFetchers implements QueryFetchers {
    ...
    public Map<String, DataFetcher> getQueryFetchers() {
        return Map.ofEntries(
                ...
                Map.entry("findMultipleNames", findMultipleNames())
        );
    }
    ...
    public DataFetcher<findMultipleNamesValue> findMultipleNames() {
        return environment -> {
            Map<String, Object> argument = environment.getArgument("input");
            final findMultipleNamesFilterInput input = inputMapper.tofindMultipleNamesFilterInput(argument);
            final findMultipleNamesNodeTypeFilterInput nodeTypeFilter = input.getNodeTypeFilter();
            final CatalogRecordSpecification catalogEntryType = specificationMapper.toCatalogRecordSpecification(nodeTypeFilter);
            return catalogVerificationService.getfindMultipleNames(catalogEntryType);
        };
    }
